...
class Jogo:
    ...
    def atualizar_tela(self):
        try:
            ...
            status_lista = self.modelo.select(f'SELECT * FROM chao WHERE jog_id = {self.id_jogador};')[0][1:-1]
            status_novo = []
            
            for i in status_lista[1:]:
                num = randint(1, 3)
                if i - num <= 0:
                    messagebox.showinfo('Game Over', 'Seu chao faleceu! o jogo será reiniciado.')
                    raise ValueError
                else:
                    status_novo.append(i-num)
            
            self.lbl_nome.config(text=f'Nome: {status_lista[0]}')
            self.lbl_carinho.config(text=f'Carinho: {status_novo[0]}%')
            self.lbl_higiene.config(text=f'Higiene: {status_novo[1]}%')
            self.lbl_diversao.config(text=f'Diversão: {status_novo[2]}%')
            self.lbl_sono.config(text=f'Sono: {status_novo[3]}%')
            self.lbl_fome.config(text=f'Fome: {status_novo[4]}%')
            
            self.janela.after(5000, self.atualizar_tela())
        except ValueError:
    ...
    #o valor adicionado da açao realizada tbm deve ter um randint entre 10 e 15 caso isso seja feito
    def agir(self, acao):
        valor_novo = int(self.modelo.select(f'SELECT {acao} FROM chao where jog_id = {self.id_jogador};')[0][0]) + randint(10, 15)
        
        if valor_novo > 100:
            valor_novo = 100
        
        self.modelo.comando(f'UPDATE chao SET {acao} = {valor_novo} WHERE jog_id = {self.id_jogador};')
